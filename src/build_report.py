...
def main() -> Path:
    ...
    md.append("")
    md.append(
        "**Alcance del análisis**: el objetivo del trabajo es **descriptivo y predictivo**, no causal. "
        "Por tanto, las asociaciones observadas no deben interpretarse como relaciones causa–efecto."
    )
    md.append("")
    md.append(
        "**Uso del análisis no supervisado**: las técnicas no supervisadas se emplean con fines **exploratorios**, "
    ...
    md.append(
        "**Nota ética y de uso**: el dataset es de uso académico/público; el análisis se presenta con fines formativos. "
    ...
    if num_cols:
        ...
        cols_show = [
            c
            for c in ["count", "mean", "std", "min", "25%", "50%", "75%", "max"]
            ...
            rows.append(
                {
                    "variable": c,
                    "n_categorías": nun,
                    "categoría_más_frecuente": top_val,
                    "frecuencia": top_n,
                }
            )
    ...
    md.append("Faltantes reales (NaN) antes de la limpieza (top 5 por columna):")
    md.append("")
    md.append(
    ...
    if semantic_before is not None:
        ...
        for _, r in top_sem.iterrows():
            ...
            if (nmiss is not None) and (pct is not None):
                rows.append(
                    f"- La variable `{var}` concentra faltantes semánticos: **{nmiss}** registros (**{_pct(pct)}** aprox.)."
                )
            elif nmiss is not None:
                rows.append(
                    f"- La variable `{var}` concentra faltantes semánticos: **{nmiss}** registros."
                )
            elif pct is not None:
                rows.append(
                    f"- La variable `{var}` concentra faltantes semánticos: **{_pct(pct)}** aprox."
                )
            else:
        ...
        md.append(
            "- Los faltantes semánticos son bajos o están concentrados en pocas variables; se aplica imputación para garantizar consistencia del análisis."
        )
    ...
    md.append("Muestra estratificada (3 filas de `<=50K` y 2 filas de `>50K`):")
    md.append("")
